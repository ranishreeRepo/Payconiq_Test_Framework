...
public class BookingStepDef{
	...
	public Response runDELETEAction(String URL) {
		try {
			response = RestAssured
					.given()
					.baseUri(URL)
	...
	public String getToken(String username, String password, String oAuthEndPoint, String payload)
	{
		token = RestAssured
		.given()
	...
	@Given("Create a valid partial update booking Payload using {string}, {string}, {int}, {string}, {string}, {string}, {string}")
	...
	public void hitupdateBookingAPI(String endpoint)
	{
		...
		try {
			...
			this.runPOSTAction(append_BookingID,payload, header);
		} catch (Exception e) {
			e.getMessage();
		}
	}
	...
	{
		String appended_ID_URL = URL+bookingId;
		this.runDELETEAction(appended_ID_URL);
	}
